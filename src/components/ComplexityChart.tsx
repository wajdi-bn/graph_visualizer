...
function normalizeToKey(raw: string): ComplexityKey | null {
  ...
  if (/n²|n\^2|v²|v\^2/.test(s)) return 'O(n²)'
  if (/nlogn|n\*logn|\(v\+e\)log|elog|elogv|n\^1\.25/.test(s)) return 'O(n log n)'
...
function parseTimeComplexity(description: string): Entry[] {
  ...
  const idxFr = description.indexOf('Complexite temporelle')
  const idxEs = description.indexOf('Complejidad Temporal')
  const idx = idxEn !== -1 ? idxEn : idxFr !== -1 ? idxFr : idxEs
  if (idx === -1) return entries

  const isFrench = idxFr !== -1 && (idxEn === -1 || idxFr < idxEn)
  const isSpanish = idxEs !== -1 && (idxEn === -1 || idxEs < idxEn)

  const rest = description.slice(idx)
  const blockRe = isFrench
    ? /Complexite temporelle[:\s]*([\s\S]*?)(?=\n\s*\nComplexite spatiale|\n\s*\nApplications|\n\s*\n[A-Z]|$)/
    : isSpanish
  ...
  const best = block.match(oRe(isSpanish ? 'Mejor' : isFrench ? 'Meilleur' : 'Best'))
  const avg = block.match(oRe(isSpanish ? 'Promedio' : isFrench ? 'Moyen' : 'Average'))
  const worst = block.match(oRe(isSpanish ? 'Peor' : isFrench ? 'Pire' : 'Worst'))

  if (best || avg || worst) {
    if (best) {
      const k = normalizeToKey(best[1])
      if (k) entries.push({ label: isSpanish ? 'Mejor' : isFrench ? 'Meilleur' : 'Best', raw: best[1], key: k, color: '#34d399' })
    }
    if (avg) {
      const k = normalizeToKey(avg[1])
      if (k) entries.push({ label: isSpanish ? 'Prom' : isFrench ? 'Moy' : 'Avg', raw: avg[1], key: k, color: '#fbbf24' })
    }
    if (worst) {
      const k = normalizeToKey(worst[1])
      if (k) entries.push({ label: isSpanish ? 'Peor' : isFrench ? 'Pire' : 'Worst', raw: worst[1], key: k, color: '#f87171' })
    }
  } else {
  ...
  return (
    <div className="mt-5 mb-3">
      <div className="text-[10px] text-neutral-500 uppercase tracking-wider font-semibold mb-2">
        {locale === 'fr' ? 'Complexite temporelle' : 'Time Complexity'}
      </div>
